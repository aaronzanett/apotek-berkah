...
<section id="kontak">
    <div class="tabledata shadow">
        <div class="pagetitle-tabledata">
            <h2 class="pageTitle">Kontak Pelanggan</h2>
            <button class="btn edit openModalBtn tabledata-addbtn add-kontakPelanggan"><i class="bi bi-plus-lg"></i> Tambah kontak</button>
        </div>

        <div class="tabledata-header">
            <div class="tabledata-group">
                <i class="bi bi-search tabledata-searchicon"></i>
                <input type="text" class="tabledata-searchinput tabledata-kontakPelanggan-search" placeholder="Cari kontak pelanggan...">
            </div>
        </div>

        <div class="tabledata-subheader">
            <p class="tabledata-totaldata">Total <?= count($data['kontakPelanggan']) ?> kontak</p>
        </div>
        ...
        <div class="tabledata-body">
            <div class="tabledata-body-overflow-md">
                ...
                <form action="<?= BASEURL ?>/headofficeOpp/deleteManyKontakPelanggan" method="post" class="alertConfirmKontakPelanggan2">
                    <div id="tabledata-items">
                        <?php if (count($data['kontakPelanggan']) !== 0) { ?>
                            <?php $i = 1; foreach ($data['kontakPelanggan'] as $kontak ) : ?>
                                <div class="tabledata-item">
                                    ...
                                    <div class="tabledata-actions">
                                        <div class="action openModalBtn detail-kontakPelanggan" data-id="<?= $kontak['id'] ?>"><i class="bi bi-eye detail-icon"></i></div>
                                        <div class="action openModalBtn edit-kontakPelanggan" data-id="<?= $kontak['id'] ?>"><i class="bi bi-pencil"></i></div>
                                        <a href="<?= BASEURL ?>/headofficeOpp/deleteKontakPelanggan/<?= $kontak['id'] ?>" class="action alertConfirmKontakPelanggan"><i class="bi bi-trash"></i></a>
                                    </div>
                                </div>
                            <?php endforeach;?>
                            <?php } else { ?>
                ...
                <?php if (count($data['kontakPelanggan']) !== 0) { ?>
                    <div class="tabledata-footer">
                        <div class="choose-items">
                            ...
                            <button type="submit" class="btn delete tabledata-choose-delete d-none alertConfirmKontakPelanggan">Hapus</button>
                        </div>
                    </div>
                    <?php } else {} ?>
                </form>
        ...
        <div class="modal-content">
            ...
            <div class="modal-data">
                <div class="modal-body">
                    <form action="<?= BASEURL ?>/headofficeOpp/addKontakPelanggan" method="post">
...
$(function () {
    //add 
    $(".add-kontakPelanggan").on("click", function () {
        $(".modal-title").html("Tambah kontak pelanggan baru");
        $(".modal-footer button[type=submit]").html("Tambah");
        $(".modal-body form").attr("action", "<?= BASEURL ?>/headofficeOpp/addKontakPelanggan" );
    ...
    function crudOperation(){
        ...
        $('.alertConfirmKontakPelanggan').on('click', function(e) {
            const href = $(this).attr('href');
            e.preventDefault();
            Swal.fire({
            title: "Apakah Anda Yakin?",
            text: "Semua akun pengguna yang berkaitan dengan Pelanggan ini akan ikut terhapus!",
            icon: "warning",
        ...
        $('.alertConfirmKontakPelanggan2').on('submit', function(e) {
            e.preventDefault();
            const form = this;
            Swal.fire({
                title: "Apakah Anda Yakin?",
        ...
        // edit
        $(".edit-kontakPelanggan").on("click", function () {
            $(".modal-title").html("Edit kontak pelanggan");
            $(".modal-footer button[type=submit]").html("Edit");
            $(".modal-body form").attr("action","<?= BASEURL ?>/headofficeOpp/editKontakPelanggan");
            $('.configAlert').addClass('d-none')
            $(".modal-footer button[type=submit]").removeClass('disabled')
            ...
            $.ajax({
            url: "<?= BASEURL ?>/headofficeOpp/getDataEditKontakPelanggan",
            data: {id: id},
            method: "post",
            dataType: "json",
        ...
        // detail
        $('.detail-kontakPelanggan').on('click', function() {
            $(".modal-title").html("Detail kontak pelanggan");
            $('.configAlert').addClass('d-none')
            $(".modal-footer button[type=submit]").removeClass('disabled')
            const id = $(this).data("id");
            ...
            $.ajax({
                ...
                dataType: "json",
                success: function (data) {
                    let kontakPelangganDetail = `<table class="table-detail">
                        <tr>
                    ...
                    $('.modal-detail-body').html(kontakPelangganDetail)
                },
            });
    ...
    $('#phonenumber').on('keyup', function() {
        ...
        $.ajax({
            url: "<?= BASEURL ?>/headofficeOpp/getKontakPelangganConfig",
    ...
    // search
    $('.tabledata-kontakPelanggan-search').on('keyup', function() {
        ...
        $.ajax({
        url: "<?= BASEURL ?>/headofficeOpp/searchKontakPelanggan",
        ...
        success: function (data) {
            ...
            data.forEach(contact => {
                contacts += `<div class="tabledata-item">
                            ...
                            <div class="tabledata-actions">
                                <div class="action openModalBtn detail-kontakPelanggan" data-id="` + contact.id + `"><i class="bi bi-eye detail-icon"></i></div>
                                <div class="action openModalBtn edit-kontakPelanggan" data-id="` + contact.id + `"><i class="bi bi-pencil"></i></div>
                                <a href="<?= BASEURL ?>/headofficeOpp/deleteKontakPelanggan/` + contact.id + `" class="action alertConfirmKontakPelanggan"><i class="bi bi-trash"></i></a>
                            </div>
                        </div>`;
            })
