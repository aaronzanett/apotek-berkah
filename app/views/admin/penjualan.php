<?php
$data['penjualan'] = $this->model('PenjualanModel')->getAllPenjualanByIdOutlet($_SESSION['outlet_id']);
?>
...
<div class="tabledata shadow">
        ...
        <div class="tabledata-header">
            ...
            <div class="filter-rentangWaktu">
                <div class="form-group">
                    <label for="startDate">Tanggal awal</label>
                    <input type="date" class="startDate filter" id="startDate">
                </div>
                <div class="form-group">
                    <label for="endDate">Tanggal akhir</label>
                    <input type="date" class="endDate filter" id="endDate">
                </div>
            </div>
        </div>
...
<script>
    ...
    // search
    $('.tabledata-penjualan-search').on('input', showFilteredData);
    $('.startDate').on('change', showFilteredData);
    $('.endDate').on('change', showFilteredData);

    function showFilteredData(){
        let keyword = $('.tabledata-penjualan-search').val();
        let outlet_id = <?= $_SESSION['outlet_id'] ?>;
        let tanggal_awal = $('.startDate').val() || 'kosong';
        let tanggal_akhir = $('.endDate').val() || 'kosong';
        
        $.ajax({
            url: "<?= BASEURL ?>/adminOpp/searchPenjualanByIdOutlet",
            data: { outlet_id: outlet_id, keyword: keyword, tanggal_awal: tanggal_awal, tanggal_akhir: tanggal_akhir},
            method: "post",
            dataType: "json",
            success: function (data) {
                let i = 1;
